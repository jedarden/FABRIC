...
- **Label precedence** — `needle.host` beats `service.instance.id` in every placement combination (resource × record level: the full matrix is pinned on `/v1/logs`, with record-beats-resource cases also pinned on `/v1/traces` and `/v1/metrics`), and the losing attribute never becomes a series in any host-labeled metric family (`fabric_event_count`, `fabric_ingest_rate_per_second`, `fabric_active_workers`).
